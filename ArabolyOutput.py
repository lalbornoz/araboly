...
class ArabolyOutput(ArabolyTypeClass):
    ...
    def dispatch_pass(self, channel, context, output, src, **params):
        ...
        if context.state == ArabolyGameState.PROPERTY:
            ...
            delay += 1.250
            output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "{} leaves auction for {}!".format(src, context.board[context.auctionProperty["field"]]["title"])]}]
            if newHighestBid != 0:
                if params["newAuctionEnd"]:
                    delay += 1.250
                    output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "{} buys {} for ${}!".format(newHighestBidder, context.board[context.auctionProperty["field"]]["title"], newHighestBid)]}]
                    delay += 1.250
                    output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "{}: roll the dice!".format(context.players[params["newPlayerCur"]])]}]
                else:
                    delay += 1.250
                    output += [{"type":"message", "delay":delay, "cmd":"PRIVMSG", "args":[channel, "Current highest bid: {} at ${}".format(newHighestBidder, newHighestBid)]}]
                ...
                if params["newAuctionEnd"]:
                    delay += 1.250
                    output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "The bank retains {}!".format(context.board[context.auctionProperty["field"]]["title"])]}]
                    delay += 1.250
                    output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "{}: roll the dice!".format(context.players[params["newPlayerCur"]])]}]
                else:
                    delay += 1.250
                    output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "No bids have been placed yet!"]}]
        params["newInhibitUntil"] = time.time() + delay
        return {"channel":channel, "context":context, "output":output, "src":src, **params}
    # }}}
    ...
    def dispatch_start(self, channel, context, output, players, src, **params):
        output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "Starting Araboly game with {} players!".format(players)]}]
        params["newInhibitUntil"] = time.time() + 1.250
    ...
    def dispatch_stop(self, channel, output, **params):
        output += [{"type":"message", "delay":1.250, "cmd":"PRIVMSG", "args":[channel, "Stopping current Araboly game!"]}]
        return {"channel":channel, "output":output, **params}
    # }}}
