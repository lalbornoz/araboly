    ...
    def _leave(channel, context, output):
        for tradeKey, tradeState in context.tradeState.items():
            ...
            and not tradeKey.endswith("\0" + src):
                ...
                output = ArabolyFree._push_output(channel, context, output, "Cancelling outstanding {} from {} to {} for {}!".format(typeString, tradeState["from"], tradeState["to"], tradeState["title"]))
        ...
        or int(args[2]) == 0:
            ...
            if status:
                ...
                or context.players["byName"][tradeTo]["wallet"] <= price:
                    ...
                    if offerType == "buy":
                        output = ArabolyFree._push_output(channel, context, output, "{otherPlayer}: {src} {offerString} to buy {title} from you at ${price}! Accept, counter-offer, or reject?".format(**{"offerString":offerString, **tradeState}))
                    elif offerType == "sell":
                        output = ArabolyFree._push_output(channel, context, output, "{otherPlayer}: {src} {offerString} to sell {title} to you at ${price}! Accept, counter-offer, or reject?".format(**{"offerString":offerString, **tradeState}))
                    if counterOffer:
    ...
    def _process_offer_reply(args, channel, cmd, context, output, src, status):
        ...
        or not args[0] in context.players["byName"]:
            ...
            if tradeKey not in context.tradeState:
                ...
                if tradeState["counter"]:
                    output = ArabolyFree._push_output(channel, context, output, "{src}: {replyTypeString}! {otherPlayer} {replyType}s your counter-offer to {offerType} {title} at ${price}!".format(**tradeState))
                else:
                    output = ArabolyFree._push_output(channel, context, output, "{src}: {replyTypeString}! {otherPlayer} {replyType}s your offer to {offerType} {title} at ${price}!".format(**tradeState))
                if replyType == "accept":
                    output = ArabolyFree._push_output(channel, context, output, "Awfom! {} buys {} for ${}!".format(tradePropTo, tradeState["title"], tradeState["price"]), 0.900)
                    ...
                    context, _, output = ArabolyFree._prop_recv(channel, context, context.board[tradeState["field"]], output, tradePropTo, tradeState["price"])
        return args, channel, cmd, context, output, src, status
    # }}}
