...
class ArabolyFree(ArabolyTypeClass):
    ...
    def dispatch_status(args, channel, context, output, src, status):
        ...
        if status:
            ...
            output = ArabolyGenerals._push_output(channel, context, output, "Araboly status for player {statusPlayer}:".format(**locals()), outputLevel=ArabolyOutputLevel.LEVEL_GRAPHICS)
            output = ArabolyGenerals._push_output(channel, context, output, "Field....: {playerField[title]}".format(**locals()), outputLevel=ArabolyOutputLevel.LEVEL_GRAPHICS)
            output = ArabolyGenerals._push_output(channel, context, output, "Wallet...: ${playerWallet}".format(**locals()), outputLevel=ArabolyOutputLevel.LEVEL_GRAPHICS)
            if len(playerProps):
                output = ArabolyGenerals._push_output(channel, context, output, "Properties owned:", outputLevel=ArabolyOutputLevel.LEVEL_GRAPHICS)
            ...
            output = ArabolyGenerals._push_output(channel, context, output, "Current turn: {}".format(context.players["numMap"][context.players["curNum"]]), outputLevel=ArabolyOutputLevel.LEVEL_GRAPHICS)
        return args, channel, context, output, src, status
    # }}}
